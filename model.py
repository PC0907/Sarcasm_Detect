...
class Model(nn.Module):
    def __init__(self, bert):
        ...
        self.embed_size = config.embed_size
        self.linear = nn.Linear(38, 2)  # Changed to emit scores for CRF
        self.use_glove = config.use_glove
        self.uw = nn.Parameter(torch.FloatTensor(torch.randn(100)))
        self.crf = CRF(num_tags=2)  # Added CRF layer
    ...
    def forward(self, x_batch, tags=None):
      ...
      emissions = self.linear(f_a)
      #print(f"Emissions shape before unsqueeze: {emissions.shape}")
      emissions = emissions.unsqueeze(1)
      #print(f"Emissions shape after unsqueeze: {emissions.shape}")
      
      if tags is not None:
          #print(f"Tags shape before unsqueeze: {tags.shape}")
          tags = tags.unsqueeze(1)
          #print(f"Tags shape after unsqueeze: {tags.shape}")
          mask = torch.ones_like(tags, dtype=torch.bool)
          #print(f"Mask shape: {mask.shape}")
          return self.crf(emissions, tags, mask)
      else:
          mask = torch.ones(emissions.shape[:2], dtype=torch.bool, device=emissions.device)
          #print(f"Mask shape (inference): {mask.shape}")
          return self.crf(emissions, mask=mask)